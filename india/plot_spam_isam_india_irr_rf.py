...
isam=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/rice_cheyenne/his_cru/india/ric_irr_fert/output/ric_irr_fert.nc','r')
grow = isam.variables['totalyield'][103:105,:,:]
lonisam = isam.variables['lon'][:]
latisam = isam.variables['lat'][:]

ryield=N.average(grow,axis=0)
ryield,lona11=shiftgrid(180.5,ryield,lonisam1,start=False)

isam1=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/rice_cheyenne/his_cru/india/ric_fert/output/ric_fert.nc','r')
grow = isam1.variables['totalyield'][103:105,:,:]

ryieldr=N.average(grow,axis=0)
ryieldr,lona11=shiftgrid(180.5,ryieldr,lonisam1,start=False)
...
spamy = spam.variables['ricey_irrigated'][:,:]
spamya = spam.variables['riceya_irrigated'][:,:]
spamp = spam.variables['ricep_irrigated'][:,:]
spama = spam.variables['ricea_irrigated'][:,:]

rspamy = spam.variables['ricey_rainfed'][:,:]
rspamya = spam.variables['riceya_rainfed'][:,:]
rspamp = spam.variables['ricep_rainfed'][:,:]
rspama = spam.variables['ricea_rainfed'][:,:]
...
rspamy[N.isnan(rspamy)]=0.
rspamy[N.isinf(rspamy)]=0.

ryieldr= ma.masked_where(ryieldr<=0.0,ryieldr)
ryieldr= ma.masked_where(rspamy<=0.0,ryieldr)
rspamy= ma.masked_where(ryieldr<=0.0,rspamy)
rspamy= ma.masked_where(rspamy<=0.0,rspamy)
rspamy= ma.masked_where(rspamy>=10.0**20,rspamy)

ryieldr= ma.masked_where(rspamy<=0.0,ryieldr)
rspamy= ma.masked_where(ryieldr<=0.0,rspamy)
ryieldr= ma.masked_where(ind<=0.0,ryieldr)
rspamy= ma.masked_where(ind<=0.0,rspamy)

aa=ryield-ryieldr
aa= ma.masked_where(aa==0.0,aa)

cs1 = map.pcolormesh(x,y,aa/ryield*100,cmap=plt.cm.bwr,vmin=-90,vmax=90)
plt.axis('off')
...
cs1 = map.pcolormesh(x,y,bb/spamy*100,cmap=plt.cm.bwr,vmin=-90,vmax=90)
plt.axis('off')
...
plt.savefig('riceisam_spam_india_irr_rf.jpg',bbox_inches='tight')
