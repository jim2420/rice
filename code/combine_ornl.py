...
def yieldout(year):
	file1=NetCDFFile('globalmapirrarea4.1.nc','r')
	maii=file1.variables['Band1'][:,:]
	maii=maii/100
	marr=1.0-maii
        maii= ma.masked_where(maii<=0,maii)
        maii= ma.masked_where(maii>=5000,maii)
        maii=ma.filled(maii, fill_value=0.)
        marr= ma.masked_where(marr<=0,marr)
        marr= ma.masked_where(marr>=5000,marr)
        marr=ma.filled(marr, fill_value=0.)
	...
	clm2=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/rice_cheyenne/his_cru/egu/ric_irr_fert/output/ric_irr_fert.nc','r')
...
#	clmtropfi= ma.masked_where(maitropi<=0,clmtropfi)
	...
	yield_clmtf = ma.masked_where(marr<=0,yield_clmtf)

	yield_clmtfi=maskoceans(x,y,yield_clmtfi)
	yield_clmtfi = ma.masked_where(maii<=0,yield_clmtfi)

#	clmy=((yield_clmtf*marr*gridarea)+(yield_clmtfi*maii*gridarea))/((marr*gridarea)+(maii*gridarea))
        clmy=(yield_clmtf*(1.0-maii))+(yield_clmtfi*maii)
...
ncfile=NetCDFFile('isamhiscru_rice_eguornl_m3.nc','w',format='NETCDF3_64BIT_OFFSET')
