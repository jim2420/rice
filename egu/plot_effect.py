...
isam=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/rice_cheyenne/his_cru/egu/fixedclimatenew/output/fixedclimatenew.nc','r')
#isam=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/rice_cheyenne/his_cru/egu/ric_irrfixed_fert/output/ric_irrfixed_fert.nc','r')
...
yclinoi=(base-base_i_cli)/(10**6) #varclimate_I-varclimate_fixedI from conclimate

yf=(base-base_f)/(10**6)
ycli=(base-base_cli)/(10**6)
yc=(base-base_c)/(10**6)
...
bar_width = 0.15
opacity = 0.8
rects3 = plt.bar(index, ayc, bar_width, yerr=syc,
         alpha=opacity,color='blue',
         label='CO2')
rects1 = plt.bar(index+bar_width, ayclinoi, bar_width, yerr=syclinoi,
         alpha=opacity,color='black',
         label='ConIrri')
rects2 = plt.bar(index+bar_width*2, aycli, bar_width, yerr=sycli,
         alpha=opacity,color='green',
         label='Cli_T')
rects0 = plt.bar(index+bar_width*3, ayirr, bar_width, yerr=syirr,
         alpha=opacity,color='yellow',
         label='Irri')
rects4 = plt.bar(index+bar_width*4, ayf, bar_width, yerr=syf,
         alpha=opacity,color='pink',
         label='NF')


plt.ylabel('Effect on prodiction (Mt/yr)',fontsize=16)
plt.tick_params(axis='both',labelsize=16)
plt.xticks(index + bar_width+0.2, ('1980s','1990s','2000s'))
leg=plt.legend(loc=1)
leg.get_frame().set_alpha(0.5)

plt.tight_layout()

plt.savefig('rice_effect.png')
plt.show()
