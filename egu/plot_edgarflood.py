...
ch4obs= ma.masked_where(ch4obs<=0.0,ch4obs)
ch4= ma.masked_where(ch4obs<=0.0,ch4)
ch4obs= ma.masked_where(ch4<=0.0,ch4obs)
...
cmap2.set_over('#232963')
cmap2.set_under('green')

bounds=[0,1,5,10,20,40,60,90,130]
bounds=[0,0.001,0.005,0.01,0.02,0.04,0.06,0.09,0.13]
...
cs1 = map.pcolormesh(x,y,ch4*mearea/gridarea,cmap=cmap,norm=norm1)
plt.axis('off')
...
ch4obs= ma.masked_where(ind!=8.0,ch4obs)
a1=ch4*mearea/(10**9)
ax1.set_title("ISAM (million t CH4/gridcell/yr)",fontsize=20)
cs1 = map.pcolormesh(x,y,a1/(10**3),cmap=cmap2,norm=norm2)
plt.axis('off')



ax1 = fig.add_subplot(224)
ax1.set_title("EDGAR (million t CH4/gridcell/yr)",fontsize=20)
map = Basemap(projection ='cyl', llcrnrlat=-15, urcrnrlat=40,llcrnrlon=55, urcrnrlon=145, resolution='c')
...
cs1 = map.pcolormesh(x,y,a2/(10**3),cmap=cmap2,norm=norm2)
plt.axis('off')
cbar = map.colorbar(cs1,location='right',size="5%",pad="2%")
cbar.ax.tick_params(labelsize=16)

plt.savefig('isam_rice_ch4flood3unit.jpg',bbox_inches='tight')
