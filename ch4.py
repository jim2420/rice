...
for i, year in enumerate(years):
    base = NetCDFFile ("/scratch2/scratchdirs/tslin2/isam/rice_cheyenne/his_cru/ric_irr_fert/output/ric_irr_fert.bgc-yearly-2d_{0}.nc".format(year), mode='r')
   
    yield1 = base.variables["ch4_flux"][:]
    grow[i, :, :] = yield1[:,:]
    yield2 = base.variables["ch4_oxid"][:]
    grow1[i, :, :] = yield2[:,:]
ryield=N.average(grow,axis=0)

oxid=N.average(grow1,axis=0)
lonisam=base.variables['lon'][:]
latisam=base.variables['lat'][:]



ryield,lona11 = shiftgrid(180.5,ryield,lonisam,start=False)
#print lona11
...
plt.savefig('isam_ricech4_avg_irr.jpg',bbox_inches='tight')
